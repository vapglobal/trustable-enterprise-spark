...
- [ ] Replace every filter surface with one compact, reusable searchable multi-select tree: global inline search, nested facets, per-branch search, counts, select-all/clear, recommended defaults, custom Other values, keyboard navigation, screen-reader labels, and an ellipsis trigger with tooltip
- [ ] Rebuild the landing-page creation loop with readable first-viewport typography and the sequence: create something useful, prove its value automatically, build the next one, share the success, and watch the next—or all the rest—build themselves
- [ ] Replace the Architecture page’s disconnected list with a compact, animated, color-coded graph map; add node drill-downs, tooltips, deeper shadows, Live/Reference truth labels, and validate the design against the available CareerCaptain resume-graph patterns
- [ ] Verify light, dark, desktop, and mobile layouts with automated and browser-level regression checks for the filter tree, landing page, Flow, OASA Console, and Architecture page
- [ ] Connect OASA Console action rows to their real callable server actions where contracts permit; show accurate idle, loading, success, validation, denied, and provider/error states without simulated responses
